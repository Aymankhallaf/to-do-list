<?php

$globalUrl = 'http://localhost:8080';

$errors = [
    'csrf' => 'Votre session est invalide.',
    'referer' => 'D\'où venez vous ?',
    'unvalid'=>"unvalide request",
    'invalid_task_id' => 'invalid task id',
    'archive_ko' => 'Archieved failed',
    'nb_char_ko' => "Unvalide number of characters",
    "no-request" => "there is no request",
    "update_priority_ko" => "update periorty value failed",
    'delete_ko' => "Deleted failed"
];

$messages = [
    'archive_ok' => 'The task has been archieved',
    'insert_ok' => 'Task inserted sucessfully',
    "update_periorty_ok" => "update periorty value success",
    "delete_ok"=> "deleted successfull"
];
